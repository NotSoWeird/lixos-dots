...
let
  ...
  fs = map (f: { ntfs3 = "ntfs"; }.${f} or f) config.wave.system.fs.support;
in
{
  options.wave.system.fs.support = mkOption {
    type = listOf str;
    default = [
      "vfat"
      "ext4"
    ...
    (mkIf (fs == [ ]) {
      warnings = [
        ''
          ...
          Please set {option}`config.wave.system.fs` in your configuration to remedy this.
        ''
      ];
    })
  ];
